...
exports.up = async function(knex) {
  ...
  await knex.schema.table('users', function(table) {
    ...
    table.boolean('dm_enabled').defaultTo(false).comment('Включен ли режим работы в ЛС');
  ...
  // 2. Для ВСЕХ существующих пользователей включаем режим канала
  // (чтобы не сломать текущую работу бота)
  await knex('users').update({ channel_enabled: true });
};
