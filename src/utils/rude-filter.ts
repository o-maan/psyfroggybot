...
function isKeyboardSpam(text: string): boolean {
  ...
  if (/(.)\1{2,}/.test(normalized) && normalized.length === normalized.match(/(.)\1*/)?.[0].length) {
    botLogger.debug({ text: normalized }, '🚫 СПАМ: повторы одного символа');
    return true;
  }

  // 2. ПРОВЕРКА СЛОВАРЯ - ПЕРВЫМ ДЕЛОМ!
  // Если слово есть в словаре - это автоматически НЕ спам, независимо от других правил
  if (russianWordsSet.has(normalized)) {
    botLogger.debug({ text: normalized }, '✅ НЕ СПАМ: найдено в словаре');
    return false;
  }

  // 3. ЯВНЫЙ СПАМ - только для слов НЕ из словаря

  // Клавиатурные ряды (с исключением для "ыы")
  // ПОЛНЫЕ ряды клавиатуры - только для слов 4+ букв (чтобы не ловить "мяч", "час" и т.д.)
  if (normalized !== 'ыы' && normalized.length >= 4 &&
      (/^[йцукенгшщзхъ]+$/.test(normalized) || // верхний ряд ПОЛНЫЙ
       /^[фывапролджэ]+$/.test(normalized) || // средний ряд ПОЛНЫЙ
       /^[ячсмитьбюё]+$/.test(normalized))) { // нижний ряд ПОЛНЫЙ
    botLogger.debug({ text: normalized }, '🚫 СПАМ: клавиатурные ряды');
    return true;
  }

  // ПРИМЕЧАНИЕ: Проверка смешанных клавиатурных рядов удалена
  // Слишком много ложных срабатываний - русский язык естественно чередует ряды клавиатуры

  // Только согласные 4+ подряд
  if (/^[бвгджзйклмнпрстфхцчшщъь]{4,}$/.test(normalized)) {
    botLogger.debug({ text: normalized }, '🚫 СПАМ: только согласные 4+');
    return true;
  }
  ...
  if (/^[ыъь]/.test(normalized) && normalized !== 'ыы') {
    botLogger.debug({ text: normalized }, '🚫 СПАМ: начинается с ы/ъ/ь');
    return true;
  }

  // 4. КОРОТКИЕ СЛОВА (2-3 буквы) - по умолчанию НЕ спам
  // Убрана проверка whitelist - слишком много ложных срабатываний
