...
export function adaptTextForGender(text: string, gender: 'male' | 'female' | 'unknown' | null): string {
  ...
  const replacements: Array<[RegExp, string | ((...args: any[]) => string)]> = [
    ...
    // 1. СПЕЦИАЛЬНЫЕ глаголы на -ог/-ёг → -огла/-ёгла (ПЕРЕД универсальными!)
    [/ты (не )?смог(?=\s|$|[,!?.;])/gi, 'ты $1смогла'],
    [/ты (не )?мог(?=\s|$|[,!?.;])/gi, 'ты $1могла'],

    // 2. Глаголы на -шел → -шла (ПЕРЕД -ел, иначе пришел станет пришела!)
    [/ты (не )?([а-яё]+)шел(?=\s|$|[,!?.;])/gi, (match, neg, verb) => `ты ${neg || ''}${verb}шла`],
    // Примеры: пришел→пришла, нашел→нашла, ушел→ушла

    // 3. Специальные случаи: был, стал, пропал и т.д.
    ...
    // 10. УНИВЕРСАЛЬНЫЕ: глаголы БЕЗ "ты" (для случаев типа "ты при этом испытывал", кнопок, вопросов)
