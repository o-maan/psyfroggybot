...
describe('Циклическая ротация утренних сообщений', () => {
  ...
  it('должен циклически выдавать будние тексты и сбрасывать индекс после последнего', async () => {
    ...
    for (let i = 0; i < weekdayCount + 5; i++) {
      const text = await getMorningMessageText(testUserId, 2); // Вторник
      ...
      // После полного цикла начинается новый (индекс = 1, т.к. уже взяли текст[0])
      if (i === weekdayCount) {
        expect(indexes?.weekday_index).toBe(1);
        console.log('✅ Новый цикл начался корректно (индекс = 1 после взятия текста[0])');
      }
    }
  });

  it('должен циклически выдавать выходные тексты и сбрасывать индекс после последнего', async () => {
    const messages = await parseMorningMessages();
    ...
    for (let i = 0; i < weekendCount + 5; i++) {
      const text = await getMorningMessageText(testUserId, 6); // Суббота
      const indexes = getMorningMessageIndexes(testUserId);

      console.log(`Итерация ${i + 1}: weekend_index = ${indexes?.weekday_index}, текст получен: ${text.length > 0}`);
  ...
  it('должен правильно обрабатывать переход на 32-й текст выходных', async () => {
    const messages = await parseMorningMessages();
    const weekendCount = messages.weekend.length;
    ...
    const text1 = await getMorningMessageText(testUserId, 6);
    ...
    const text2 = await getMorningMessageText(testUserId, 6);
    ...
    const text3 = await getMorningMessageText(testUserId, 6);
    indexes = getMorningMessageIndexes(testUserId);
  ...
  it('должен правильно обрабатывать переход на 63-й текст будних', async () => {
    const messages = await parseMorningMessages();
    const weekdayCount = messages.weekday.length;
    ...
    // Получаем предпоследний текст
    const text1 = await getMorningMessageText(testUserId, 2);
    let indexes = getMorningMessageIndexes(testUserId);
    ...
    // Получаем последний текст
    const text2 = await getMorningMessageText(testUserId, 2);
    indexes = getMorningMessageIndexes(testUserId);
    ...
    // Получаем первый текст нового цикла
    const text3 = await getMorningMessageText(testUserId, 2);
    indexes = getMorningMessageIndexes(testUserId);
  ...
  it('не должен выдавать undefined или пустой текст на границах', async () => {
    const messages = await parseMorningMessages();
    const weekendCount = messages.weekend.length;
    ...
    for (const index of criticalIndexes) {
      ...
      const text = await getMorningMessageText(testUserId, 6);
