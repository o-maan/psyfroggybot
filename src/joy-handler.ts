...
/**
 * JoyHandler - обработчик интерактивной логики "Источники радости и энергии" (JOY логика)
 *
 * ⚠️ ВАЖНО: JOY логика ≠ "Плюшки для лягушки"!
 *
 * JOY логика - это работа с ПОСТОЯННЫМ СПИСКОМ источников радости и энергии ⚡️
 * Пользователь создаёт накопительный список того, что его заряжает и радует в жизни в целом.
 * Этот список хранится в БД и используется для напоминаний и мотивации.
 *
 * "Плюшки для лягушки" - это ЗАДАНИЕ #2 в вечерней лягушке, где пользователь описывает
 * приятные события КОНКРЕТНО ЗА СЕГОДНЯШНИЙ ДЕНЬ (разовый ответ на каждый день).
 *
 * Логика работы JOY:
 * 1. Пользователь вызывает /joy (или автоматически в воскресенье ВМЕСТО вечернего поста)
 ...
 * 8. В воскресенье: после завершения JOY → продолжается обычная вечерняя логика в том же треде
 */
...
ВЕРНИ ТОЛЬКО JSON, без объяснений.`;
      ...
      // Очищаем ссылку на кнопку (сама кнопка удаляется в callback обработчике)
