...
export async function handleMorningRespond(ctx: BotContext) {
  try {
    ...
    const readingMessage = await ctx.telegram.sendMessage(chatId, 'Читаю, минутку… 🧐', {
    ...
    botLogger.info({ userId, messagesCount: messages.length }, 'Анализируем сообщения пользователя');

    // Анализируем эмоции с помощью LLM (с низкой температурой для точности)
    const analyzePromptTemplate = readFileSync('assets/prompts/morning-analyze-emotions.md', 'utf-8');
    const analyzePrompt = analyzePromptTemplate.replace('{{USER_MESSAGES}}', userMessages);
    ...
    // Если названо 3 и более эмоций - сразу переходим к ШАГу 3
...
2. "question_text": Мягкий вопрос или просьба рассказать о чувствах, УЧИТЫВАЯ КОНТЕКСТ сообщений (чтобы вопрос звучал естественным продолжением)

Пример структуры ответа:
{
  "support_text": "Мне очень жаль это слышать! 😔 Обнимаю 👐🏻",
  "question_text": "Расскажи мне, пожалуйста, что ты почувствовал?"
}
...
- Вопрос должен быть УМЕСТЕН ситуации
- Верни ТОЛЬКО валидный JSON без дополнительных пояснений`;

      } else if (needsMoreEmotions) {
        responsePrompt = `Пользователь поделился негативными переживаниями и назвал 1-2 эмоции, но можно попросить описать подробнее.
...
2. "question_text": Мягкая просьба назвать больше эмоций или описать подробнее, УЧИТЫВАЯ КОНТЕКСТ сообщений

Пример структуры ответа:
{
  "support_text": "Это действительно неприятно, понимаю 🫠 Я рядом!",
  "question_text": "Давай попробуем вместе более подробно описать твои эмоции 🥺"
}
...
- Как человек, а не робот
- НЕ используй обращения типа "брат", "братан", "бро", "слушай" и т.п.
- Просьба должна быть УМЕСТНА ситуации
- Верни ТОЛЬКО валидный JSON без дополнительных пояснений`;
      }
    } else {
      // Позитивные события
...
2. "question_text": Вопрос о чувствах, УЧИТЫВАЯ КОНТЕКСТ сообщений (чтобы вопрос звучал естественным продолжением)

Пример структуры ответа:
{
  "support_text": "Вау, меня радуют такие новости! 😊 Это очень здорово!",
  "question_text": "Что ты испытал при этом?"
}
...
- Верни ТОЛЬКО валидный JSON без дополнительных пояснений`;
        ...
        responsePrompt = `Пользователь поделился позитивными событиями и назвал 1-2 эмоции, но можно попросить рассказать больше.
...
2. "question_text": Просьба назвать больше эмоций, УЧИТЫВАЯ КОНТЕКСТ сообщений

Пример структуры ответа:
{
  "support_text": "Радуют твои успехи! 🤩 Я горжусь тобой!",
  "question_text": "Давай попробуем назвать больше этих классных эмоций ☺️"
}
