    ...
    if (analysisData.emotions_count >= 3) {
      ...
      try {
        ...
        const negativeFallbacks = [
          'Вижу, как тебе сейчас непросто... Ты большой молодец, что смог${:ла} разобраться в своих чувствах и поделиться ими. Это важный шаг! 💚 Я рядом',
...
- Верни ТОЛЬКО валидный JSON без дополнительных пояснений`;
    ...
    try {
      ...
      // Получаем пол пользователя для адаптации fallback сообщений
      ...
      if (analysisData.sentiment === 'negative') {
        if (needsEmotions) {
          ...
          const fallbacks = [
            {
              support_text: 'Мне очень жаль это слышать! 😔 Обнимаю 👐🏻',
              question_text: `\nРасскажи мне, пожалуйста, какие эмоции ты испытал${ending(userGender, 'а')}?`
            },
            {
          ...
          const fallbacks = [
            {
              ...
              question_text: `\nПопробуй определить, какие еще чувства ты испытал${ending(userGender, 'а')}?`
            },
            {
        ...
        if (needsEmotions) {
          ...
          const fallbacks = [
            {
              support_text: 'Вау, меня радуют такие новости! 😊 Это очень здорово!',
              question_text: `\nКакие эмоции ты испытал${ending(userGender, 'а')} при этом?`
            },
            {
              support_text: 'Как приятно это слышать! 🌟 Рад за тебя!',
              question_text: `\nРасскажи, какие чувства ты почувствовал${ending(userGender, 'а')}? Назови их`
            }
          ];
          responseData = fallbacks[Math.floor(Math.random() * fallbacks.length)];
        } else {
