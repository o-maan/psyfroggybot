...
// Функция для получения фразы с учетом пола пользователя
function getMoreEmotionsPhrase(gender: 'male' | 'female' | 'unknown' | null | undefined): string {
  // Импортируем функцию адаптации
  const { ending } = require('../../utils/adapt-text-for-code');

  const phrases = [
    'Постарайся написать больше эмоций',
    'Какие еще эмоции были?',
    `Что еще почувствовал${ending(gender, 'а')}?`,
    `Загляни в таблицу эмоций, что еще ощутил${ending(gender, 'а')}?`,
    ...
    `Прислушайся к себе - что еще ощутил${ending(gender, 'а')}?`,
    `Углубись в свои ощущения, какие еще эмоции ты испытал${ending(gender, 'а')}?`,
    'Покопайся в своих чувствах поглубже, что там еще?',
    `Что еще ты переживал${ending(gender, 'а')} в тот момент?`,
    ...
    `Назови больше того, что ты чувствовал${ending(gender, 'а')} - с таблицей будет проще`,
    `Что еще ты испытывал${ending(gender, 'а')}? Таблица будет подсказкой`,
...
// Функция для получения поддерживающего сообщения с учетом пола пользователя
function getSupportMessage(gender: 'male' | 'female' | 'unknown' | null | undefined): string {
  const { ending } = require('../../utils/adapt-text-for-code');

  const messages = [
    'Ты отлично справляешься с описанием своих чувств! Я горжусь тобой! Ты со всем справишься 💚',
    `Спасибо, что так подробно рассказал${ending(gender, 'а')} про свои переживания. Ты молодец, что смог${ending(gender, 'ла')} это выразить 🙌🏻`,
    `Я горжусь тобой! Ты очень точно описал${ending(gender, 'а')} свои чувства, это требует смелости 💙`,
    'Какая глубокая работа с эмоциями! Ты все лучше слышишь себя 🫶🏻 Это ценный навык ⚡️',
    `Спасибо за доверие и честность. Ты проделал${ending(gender, 'а')} важную работу с чувствами 💜`,
    ...
    `Отличная работа! Ты смог${ending(gender, 'ла')} выразить сложные переживания словами 👏🏻`,
    ...
    `Говорить о неприятных переживаниях - непростая работа, и ты справил${ending(gender, 'ась')}! Я рядом 👐🏻`,
    `Я ценю, что ты поделил${ending(gender, 'ась')} этим со мной. Твои чувства важны 🕊️`,
    `Спасибо, что не побоял${ending(gender, 'ась')} назвать неприятные чувства. Это смело 🔥`,
    ...
    `Ты не просто описал${ending(gender, 'а')} ситуации - ты услышал${ending(gender, 'а')} свои чувства! Ты справишься с чем угодно 💚`,
    'Я вижу, как ты работаешь над собой. Это вдохновляет! ⭐',
    `Ты смог${ending(gender, 'ла')} выразить то, что многие держат внутри. Это твоя сила 💪🏻`,
    ...
    `Ты смог${ending(gender, 'ла')} увидеть и принять свои эмоции. Я с тобой, продолжаем! 🤗`,
  ];
  ...
  try {
    ...
    if (userMessages.length > 1) {
      ...
      if (fewEmotionsCount === 0) {
        ...
        // Получаем пол пользователя для адаптации текста
        const { getUserByChatId } = await import('../../db');
        const user = getUserByChatId(userId!);
        const userGender = (user?.gender as 'male' | 'female' | 'unknown' | null) || 'unknown';

        // Генерируем поддерживающее сообщение с учетом пола
        const supportText = getSupportMessage(userGender);

        botLogger.info({ channelMessageId }, `💚 Отправляем поддерживающее сообщение`);
...
export async function sendEmotionsClarificationStep(
  ...
  if (step === 0) {
    // Первое сообщение - генерируем фразу с учетом пола
    const randomPhrase = getMoreEmotionsPhrase(userGender);
