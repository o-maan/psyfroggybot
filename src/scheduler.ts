    ...
    try {
      ...
      if (promptName === 'practice-completed') {
        const fallbacks = [
          ...
          `Великолепно! 🌈 Ты сделал${'${:а}'} важный шаг для своего благополучия.`,
          `Ты справил${'${ся:ась}'}! 🎯 На сегодня все задания выполнены.`,
          'Ты молодец! 🌙 Пора отдыхать.',
          `Я горжусь тобой! 💫 Ты сделал${'${:а}'} отличную работу.`,
          'Прекрасная работа! 🎉 Теперь можно расслабиться.',
          'Браво! 🌿 Все задания на сегодня завершены.',
          `Замечательно! ⭐ Ты проявил${'${:а}'} заботу о себе.`,
        ];
...
${errorCount > 0 ? `\n🚨 Ошибки:\n${errors.slice(0, 5).join('\n')}${errors.length > 5 ? '\n...' : ''}` : ''}`;
  ...
  private initializeDailySchedule() {
    ...
    // ОТКЛЮЧЕНО: Вечерняя рассылка теперь через timezone-based планировщик
    // this.startDailyCronJob();
    // ОТКЛЮЧЕНО: Утренняя проверка теперь через timezone-based планировщик
    // this.startMorningCheckCronJob();
    // ОТКЛЮЧЕНО: Утреннее сообщение теперь отправляется через timezone-based планировщик
    // this.startMorningMessageCronJob();
    // ОТКЛЮЧЕНО: Вечерняя batch обработка теперь через timezone-based планировщик
    // (Утренняя обработка теперь асинхронная, сразу после сохранения в message_links)
    // this.startEveningBatchProcessingCronJob();
  }

  // Запуск cron job для ежедневной отправки в 20:00
  private startDailyCronJob() {
    ...
    // Создаем новый cron job: каждый день в 20:00
    // Формат: "минуты часы * * *" (0 20 * * * = 20:00 каждый день)
    ...
    if (this.dailyCronJob) {
      schedulerLogger.info({ processId, expression: '0 20 * * *' }, 'Cron job успешно создан');
    } else {
  ...
  // Запуск cron job для утренней batch обработки в 8:45 (за 15 мин до утреннего поста)
  private startMorningBatchProcessingCronJob() {
    ...
    schedulerLogger.info('Создание morning batch processing cron job (8:45 МСК)');
    ...
    // Создаем новый cron job: каждый день в 8:45
  ...
  // Запуск cron job для вечерней ПЕРЕПРОВЕРКИ в 19:45
  // Основная обработка происходит асинхронно сразу после сохранения сообщения
  // Этот cron job доделывает то, что не успело обработаться
  private startEveningBatchProcessingCronJob() {
    ...
    schedulerLogger.info('Создание evening batch processing cron job (19:45 МСК) - ПЕРЕПРОВЕРКА необработанных');

    // Создаем новый cron job: каждый день в 19:45
    this.eveningBatchProcessingCronJob = cron.schedule(
      '45 19 * * *',
      async () => {
        schedulerLogger.info('🌆 Запуск вечерней ПЕРЕПРОВЕРКИ необработанных сообщений');
        try {
          const { processBatchMessages } = await import('./batch-processor');
          await processBatchMessages();
        } catch (error) {
  ...
  public getSchedulerStatus() {
    ...
    return {
      ...
      usersCount,
      usersList,
      cronExpression: '0 20 * * * (вечер), 0 8 * * * (утро)',
      timezone: 'Europe/Moscow',
      description: 'Ежедневно в 20:00 МСК (рассылка) и 8:00 МСК (проверка)',
    ...
    try {
      ...
      if (promptNumber === 4) {
        ...
        const prompt4Examples = [
          ...
          `Это что такое?! 🤨 Пропал${'${:а}'} без вести вместе с заданиями! Я готов объявить розыск! 🚨\nМы найдем тебя, где бы ты ни прятал${'${ся:ась}'} 🐸`,
          ...
          `Что это было?! 🤨 Вздумал${'${:а}'} дурить лягуху и не делать задания? Ну уж нееет - бегом пиши все, что нужно! 😠\nТы делаешь это для себя, не забывай 🙌🏻`,
          ...
          `Вчера ты молчал${'${:а}'}, и я от нечего делать организовал забастовку мух. Они требуют твоего возвращения! 📢 \nСделай быстрее задания ✍🏻`,
        ...
        if (isLLMError(generatedText, cleanedText)) {
          ...
          const fallbackExamples = [
            // Промпт 1 - злюсь
            'Кто-то вчера не сделал задания, что за безобразие! 😠 Я весь вечер ждал твой ответ,\nпроверял сообщения, а ты не написал${:а} ни слова! 🧐 Надо это исправить, у тебя получится 💪🏻',
            ...
            'Ээй! 😤 Ты что, забыл${:а} про меня? Я жду-жду, а ты молчишь! 🤐 Вчера весь вечер просидел у экрана в ожидании твоего сообщения 🐸  Давай, возвращайся и сделай задания',
            ...
            'Как так?! 😤 Я весь вечер ждал твой ответ! Даже чай остыл, пока сидел у экрана. Где же ты был${:а}? 😩 Неужели забыл${:а} про нашу важную работу? Возвращайся скорее',
            ...
            'Эй! 😤 Ты забыл${:а} про меня? Я жду-жду, а ты молчишь! 🐸 Давай, возвращайся',
            'Ты вчера не подели${лся:лась} своими эмоциями... 😔 Я все еще здесь и готов выслушать! Давай наверстаем 🐸',
            'Что же ты так? 🙈 Я жду, задания ждут, а ты так и не появи${лся:лась} 😒 Порадуй ляхуху своими ответами 🥺',
            ...
            'Ты ничего не написал${:а} - мне грустно без твоих историй 🐸 Поделись своими переживаниями. Я рядом 🌟',
            'Сижу, грущу в одиночестве - ты вчера ничего не написал${:а} 😓 Меня это огорчает. Давай попробуем сейчас заполнить то, что пропустили?',
            'Эхх! Вчера ты так и не появи${лся:лась} 🫤 Не забывай про лягуху. Еще можно сделать задания. Постоянство - ключ к успеху 🗝️',
            'Ты знаешь, я немного приуныл ☹️ Вчера от тебя не было ни слова. У тебя все хорошо? Давай вернемся и сделаем задания 📝',
            'Скучаю по нашим беседам 😔 Вчера ты не выполнил${:а} задания 📋 Расскажи, что случилось?',
            'Что-то на душе тяжело... 🥺 Ты вчера проигнорировал${:а} задания. Хочется видеть твои результаты, а для этого нужно постараться! 🚀 Давай вернемся и пройдем этот путь вместе 👐🏻',
            'С нетерпением жду твоих сообщений. Но вчера ты не написал${:а} 😮‍💨 Давай поговорим? Я скучаю',
            ...
            'Грущу... 😢 Ты вчера даже не заглянул${:а}. Ты же знаешь, я всегда рядом и готов выслушать 🐸 Не пропадай так',
            'Печалька... 🥺 Я так и не увидел твой ответ вчера. Поделись, что у тебя происходит? 🫂',
            'Знаешь, что самое грустное? Я даже не знаю - все ли у тебя в порядке 🥺 Ты как? Готов${:а} продолжить? ⚡️ Новые привычки формируются систематичностью. Не останавливайся 🙏🏻',
            'Смотрю на невыполненные задания 🗒️ и становится так грустно 😞 Возвращайся, давай продолжим 👨🏻‍🎓',
            'Вчера я долго смотрел на экран, надеясь увидеть твои сообщения. Но ты так и не появи${лся:лась} 😒 Нужно это исправить - сделай задания сейчас ✍🏻',
            ...
            'А почему вчера не смог${:ла} сделать задания? 📃 Я очень жду - пиши. Мне ведь важно знать, как у тебя дела 🐸',
            ...
            'Привет! 👋🏻 Вчера ты не ответил${:а}, и я весь вечер думал о тебе. Просто волнуюсь - все ли в порядке? Работа над собой - это не прямая дорога. Иногда мы останавливаемся, и это тоже часть пути. Давай продолжим вместе? 🐸',
            'Вчера от тебя не было вестей! 🥺 Ты не долж${ен:на} быть идеальн${ым:ой} каждый день! И можешь рассказать мне обо всем! Очень важно продолжать уделять время самому важному - себе! ♥️',
            'Пропустил${:а} задания, так бывает, я понимаю. Только не останавливайся! Каждый момент - это шанс начать заново 🌟 И я буду рядом, чтобы помочь тебе в этом 🤗',
            ...
            'Вчера ты не написал${:а}... Я волновался и думал о тебе. Знаешь, что хочу сказать? Спасибо, что ты вообще начал${:а} этот путь. Это требует смелости 🌱 Возвращайся поскорее! Я здесь для тебя 🐸',
    ...
    try {
      ...
      if (!captionWithComment || captionWithComment.trim().length === 0) {
        ...
        if (adminChatId) {
          await this.bot.telegram.sendMessage(
            adminChatId,
            `🚨 КРИТИЧЕСКАЯ ОШИБКА: утренний пост для пользователя ${userId} пришел БЕЗ ТЕКСТА!\n\nИспользован fallback текст.\nДень недели: ${dayOfWeek}\nПроверь файлы morning-messages.ts и assets/prompts/weekday-messages.md`
          );
        }
      }
...
- ТОЛЬКО текст поддержки, без кавычек, без технической информации`;
  ...
  public async handleInteractiveUserResponse(
    ...
    const messageLinkQuery = db.query(`
      ...
      WHERE message_id = ? AND channel_message_id = ? AND message_type = 'user'
      LIMIT 1
    `);
    ...
    try {
      ...
      if (session.currentStep === 'deep_waiting_positive') {
        ...
        let finalMessage = '<i>Вау! 🤩 Ты справил${ся:ась}! Это было потрясающе!</i>\n\n';
      ...
      if (session.currentStep === 'deep_waiting_positive_emotions_clarification') {
        ...
        const user2 = getUserByChatId(userId);
        const userGender2 = (user2?.gender || 'male') as 'male' | 'female' | 'unknown';
        const { getFixedText: getFixedText2 } = require('./utils/send-to-user');
        const buttonText2 = getFixedText2('button_practice_done', userGender2) || '✅ Сделал';

        const practiceKeyboard = {
          inline_keyboard: [
            [{ text: buttonText2, callback_data: `pract_done_${channelMessageId}` }],
            [{ text: '⏰ Отложить на 1 час', callback_data: `pract_delay_${channelMessageId}` }],
          ],
        };
            ...
            await this.sendWithRetry(
              () =>
                sendToUser(
                  ...
                  userId,
                  userId,
                  'Отличная работа! 🌟 Теперь выполни дыхательную практику и нажми "Сделал${:а}" после ее завершения'
                ),
              {
                chatId: userId,
      ...
      if (session.currentStep === 'waiting_negative') {
        ...
        const user3 = getUserByChatId(userId);
        const userGender3 = (user3?.gender || 'male') as 'male' | 'female' | 'unknown';
        const { getFixedText: getFixedText3 } = require('./utils/send-to-user');
        const buttonText3 = getFixedText3('button_practice_done', userGender3) || '✅ Сделал';

        const practiceKeyboard = {
          inline_keyboard: [
            [{ text: buttonText3, callback_data: `pract_done_${channelMessageId}` }],
            [{ text: '⏰ Отложить на 1 час', callback_data: `pract_delay_${channelMessageId}` }],
          ],
        };
        ...
        const user4 = getUserByChatId(userId);
        const userGender4 = (user4?.gender || 'male') as 'male' | 'female' | 'unknown';
        const { getFixedText: getFixedText4 } = require('./utils/send-to-user');
        const buttonText4 = getFixedText4('button_practice_done', userGender4) || '✅ Сделал';

        const practiceKeyboard = {
          inline_keyboard: [
            [{ text: buttonText4, callback_data: `pract_done_${channelMessageId}` }],
            [{ text: '⏰ Отложить на 1 час', callback_data: `pract_delay_${channelMessageId}` }],
          ],
        };
            ...
            await this.sendWithRetry(
              () =>
                sendToUser(
                  ...
                  replyToChatId,
                  userId,
                  'Выполни практику и нажми "Сделал${:а}" после ее завершения',
                  reminderOptions
                ),
              {
                chatId: userId,
...
🗓 Триггер - Мысли - Эмоции - Ощущения в теле - Поведение или импульс к действию`;
        ...
        const user5 = getUserByChatId(userId);
        const userGender5 = (user5?.gender || 'male') as 'male' | 'female' | 'unknown';
        const { getFixedText: getFixedText5 } = require('./utils/send-to-user');
        const buttonText5 = getFixedText5('button_practice_done', userGender5) || '✅ Сделал';

        const practiceKeyboard = {
          inline_keyboard: [
            [{ text: buttonText5, callback_data: `pract_done_${channelMessageId}` }],
            [{ text: '⏰ Отложить на 1 час', callback_data: `pract_delay_${channelMessageId}` }],
          ],
        };
...
💡 Одно и то же может давать и радость, и энергию 🤩`;
        ...
        if (events.length > 0) {
          ...
          schedulerLogger.info(
            { before: events.length, after: validatedEvents.length },
            '✅ Валидация позитивных событий завершена'
          );
        }
...
💡 Одно и то же может давать и радость, и энергию 🤩`;
  ...
  private async sendJoyRegularMessage(channelMessageId: number, userId: number, commentsChatId: number, events: any[]) {
    ...
    // ВАЛИДАЦИЯ: Фильтруем только ДЕЙСТВИТЕЛЬНО позитивные события через LLM
    const validatedEvents = events.length > 0 ? await this.validatePositiveEvents(events) : [];

    if (events.length > 0 && validatedEvents.length !== events.length) {
      schedulerLogger.info(
        { before: events.length, after: validatedEvents.length, removed: events.length - validatedEvents.length },
        '✅ Валидация позитивных событий завершена (убрано негатива/мусора)'
      );
    }
...
<b>Перечисли ниже ❤️‍🔥</b>`;
  ...
  private async formatEventsWithLLM(userId: number, events: any[]): Promise<string> {
    try {
      ...
      const prompt = `⚠️ СОХРАНЯЙ ТЕКСТ ПОЛЬЗОВАТЕЛЯ КАК ЕСТЬ! НЕ МЕНЯЙ ФОРМУЛИРОВКИ!

Ты лягушка-психолог (мужского рода). Пользователь делился с тобой позитивными событиями.

Твоя задача: минимальное форматирование списка (только эмоджи + строчная буква + перенос строк).
...
4. **РАЗДЕЛЯЙ НЕСКОЛЬКО СОБЫТИЙ В ОДНОМ ПУНКТЕ:**
   ...
   - ПРАВИЛЬНО разделить на 3 пункта:
...
Верни ТОЛЬКО отформатированный список событий БЕЗ изменения формулировок пользователя.`;
  ...
  /**
   * Валидация позитивных событий через LLM (белый список)
   * Оставляем ТОЛЬКО события с позитивным окрасом
   * @param events - Массив событий для валидации
   * @returns Отфильтрованный массив только позитивных событий
   */
  private async validatePositiveEvents(events: any[]): Promise<any[]> {
...
💡 Одно и то же может давать и радость, и энергию 🤩`;
  ...
  private async createCronJobsForTimezone(timezone: string, chatIds: number[]) {
    ...
    // 1. Вечерний пост: 20:00 по Москве (17:00 UTC)
