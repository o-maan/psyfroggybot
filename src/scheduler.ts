...
${errorCount > 0 ? `\n🚨 Ошибки:\n${errors.slice(0, 5).join('\n')}${errors.length > 5 ? '\n...' : ''}` : ''}`;
  ...
  async sendMorningMessage(chatId: number) {
    try {
      ...
      // ВАЖНО: Для тестового бота автоматическая рассылка отключена
      if (this.isTestBot()) {
...
3. Каждый пункт с НОВОЙ строки (без пропуска строк между пунктами)
4. Каждый пункт начинается с эмоджи 😊 (именно этот!)
5. После эмоджи текст начинается с маленькой буквы
6. **Удаляй ПОЛНЫЕ дубликаты** - если одно и то же событие повторяется несколько раз, оставь только ОДИН раз
7. НЕ добавляй заголовки, вводные фразы, комментарии
8. ТОЛЬКО список событий, ничего больше
