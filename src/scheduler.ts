...
${errorCount > 0 ? `\n🚨 Ошибки:\n${errors.slice(0, 5).join('\n')}${errors.length > 5 ? '\n...' : ''}` : ''}`;
  ...
  private initializeDailySchedule() {
    ...
    // Вечерняя batch обработка в 21:30 - ПЕРЕПРОВЕРКА необработанных сообщений
  ...
  // Запуск cron job для вечерней ПЕРЕПРОВЕРКИ в 21:30
  ...
  private startEveningBatchProcessingCronJob() {
    ...
    schedulerLogger.info('Создание evening batch processing cron job (21:30 МСК) - ПЕРЕПРОВЕРКА необработанных');
