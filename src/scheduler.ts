...
${errorCount > 0 ? `\n🚨 Ошибки:\n${errors.slice(0, 5).join('\n')}${errors.length > 5 ? '\n...' : ''}` : ''}`;
  ...
  private async sendAngryPost(userId: number) {
    ...
    try {
      ...
      // Массив fallback сообщений для злых постов (те же, что в промпте)
      const angryFallbacks = [
        'Кто-то вчера не сделал задание, что за безобразие! 😠\nНадо это исправить',
        'Вижу, что вчерашняя лягуха осталась без ответа 😔\nДавай исправим',
        ...
        'Не отлынивай, быстренько сделай вчерашние задания',
        'Ты помнишь, что ты - самое важное, что у тебя есть? 🥺 А для себя нужно постараться!\nВернись и сделай задания',
        'Психолог злиться не может, а лягуха может! 😠\nКто вчера не сделал задания? Не надо так 😒',
        'Эй! 😤 Ты что, забыл про меня?\nЯ жду-жду, а ты молчишь! 🐸 Давай, возвращайся',
        'Ну что за дела?! 😤 Вчера молчал, сегодня молчишь... Так дело не пойдет!\nДавай-давай, рассказывай про свой день! А то буду квакать под окном! 🐸',
        'Эй, человек! 👀 Ты где пропадаешь? Вчерашнее задание осталось без внимания!\nЯ переживаю за тебя... Возвращайся скорее! 💚',
        'Ква! 😡 Ты вчера не поделился своими эмоциями... 😔\nЯ все еще здесь и готов выслушать! Давай наверстаем 🐸'
      ];

      // Выбираем случайное сообщение из массива
      const randomIndex = Math.floor(Math.random() * angryFallbacks.length);
      const fallbackText = angryFallbacks[randomIndex];

      // Проверяем на ошибку и используем fallback текст
      const textToUse = (cleanedText === 'HF_JSON_ERROR' || !cleanedText) 
        ? fallbackText
        : cleanedText;

      // Ограничиваем длину текста
      const finalText = textToUse.length > 500 ? textToUse.slice(0, 497) + '...' : textToUse;
